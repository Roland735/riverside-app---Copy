...
const SubjectList = ({ subjects, teacherName }) => {
  const router = useRouter();

  const handleCardClick = (subjectName, grade) => {
    router.push(`/dashboard/teacher/subject/${subjectName}/${grade}/${2025}`);
  };

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-xl my-8 font-semibold border-b-2 border-cyan-700 text-cyan-900 dark:text-cyan-50">
        My Subjects
      </h1>
          ...
          {subjects.map((subject, index) => {
            ...
            return (
              <motion.div
                ...
                className="textColor backColor borders rounded-lg shadow-lg p-6 cursor-pointer transform transition-transform duration-200 hover:shadow-xl"
              >
                  ...
                  <div className="">
                    ...
                    <ul className="list-disc list-inside pl-6 mb-4 text-gray-600 dark:text-slate-300">
                      {subject.teachers.map((teacher, teacherIndex) => (
                        <li key={teacherIndex}>{teacherName}</li>
                      ))}
                    </ul>
                  </div>
                  <div
                ...
                <div className="mb-4">
                  <h3 className="text-lg font-semibold mb-2 text-gray-800 dark:text-cyan-50">
                    Completed Topics
                  </h3>
