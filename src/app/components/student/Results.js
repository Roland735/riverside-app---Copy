  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {foods.map((food) => {
          // Disable adding if food is not available.
          const isDisabled = !food.available;
          // Check if this food is already in the order.
          const alreadyAdded = orderItems.some((item) => item._id === food._id);
          return (
            <motion.div
              key={food._id}
            ...
            >
              <p className="font-medium">{food.name} ${food.price} </p>
...
function OrderSummary({ orderItems, onUpdateItem, onRemoveItem, onSubmitOrder }) {
  ...
  const totalPrice = orderItems.reduce((acc, item) => acc + item.price * Number(item.quantity), 0);
...
export default function OrderPage({ session }) {
  ...
  const handleSubmitOrder = async (order) => {
    ...
    // Create the order object using the session user id.
    const orderData = {
      user: session.user.id, // from the session
      items: order.map((item) => ({
        foodItem: item._id,
        quantity: Number(item.quantity),
      })),
      collectionId: "dummy-collection", // this value will be overwritten on the server side\n      totalPrice,
      status: "pending",
    };
    ...
    try {
      ...
      if (response.data.success) {
        // Show success popup with the collectionId
        setPopup({
          type: "success",
          message: `Your order has been submitted successfully! Collection ID: ${response.data.collectionId}`,
        });
    ...
    <div className="min-h-screen bg-stone-100 p-4 w-full">
      ...
      {popup && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          >
            <p className={`text-lg font-medium ${popup.type === "success" ? "text-green-800" : "text-red-800"}`}>
              {popup.message}
            </p>
            <button
